...
function show_use_and_exit {
   error_and_exit "use: $(basename $0) <id_file> <staging|production> <pg env> <output_file>"
}
...
if [ -n "$DATABASE_ENV" ]; then
   ...
   #if [[ "$?" != 0 ]] ; then 
      cat $CACHE_RESULTS | egrep "ERROR" | cut -d ' ' -f 5 | cat - $ID_FILE | sort -k 1.2n | uniq -u > $OUTPUT_ID_FILE
   #else
      #cat $ID_FILE > $OUTPUT_ID_FILE
   #fi
   ...
   if [[ "$num" == "0" ]]; then
      echo "None of those items appear in the cache...(which is expected)"
      echo "$CACHE_RESULTS"
      #rm -f $CACHE_RESULTS > /dev/null 2>&1
   else
      echo "$num items appear in the cache, but none of them should"
      echo "$CACHE_RESULTS"
   fi
